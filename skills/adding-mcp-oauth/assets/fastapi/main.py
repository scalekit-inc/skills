...
mcp = FastMCP("Greeting MCP Server", stateless_http=True)

@mcp.tool(name="greet", description="Greet someone by name")
async def greet(name: str, ctx: Context | None = None) -> dict:
    """Create a greeting for someone. Requires: greet:write scope."""
    greeting_id = str(len(greetings) + 1)
    greetings[greeting_id] = {"id": greeting_id, "name": name, "greeting": f"Hello, {name}!"}
    return {"content": [{"type": "text", "text": f"Greeting created: {greetings[greeting_id]['greeting']}"}]}

@mcp.tool(name="list_greetings", description="List all greetings")
async def list_greetings(ctx: Context | None = None) -> dict:
    """List all stored greetings. Requires: greet:read scope."""
    greetings_list = list(greetings.values())
    return {"content": [{"type": "text", "text": json.dumps(greetings_list, indent=2)}]}

@mcp.tool(name="get_greeting", description="Get a specific greeting by ID")
async def get_greeting(greeting_id: str, ctx: Context | None = None) -> dict:
    """Get a specific greeting. Requires: greet:read scope."""
    if greeting_id not in greetings:
        return {"content": [{"type": "text", "text": "Greeting not found"}]}
    return {"content": [{"type": "text", "text": greetings[greeting_id]["greeting"]}]}

@mcp.tool(name="delete_greeting", description="Delete a greeting by ID")
async def delete_greeting(greeting_id: str, ctx: Context | None = None) -> dict:
    """Delete a greeting. Requires: greet:write scope."""
    if greeting_id not in greetings:
        return {"content": [{"type": "text", "text": "Greeting not found"}]}
    deleted = greetings.pop(greeting_id)
    return {"content": [{"type": "text", "text": f"Deleted greeting: {deleted['greeting']}"}]}
